...
function LinkModal({className, open, data}: Props) {
    ...
    useEffect(() => {
        ...
        if (open) {
            gsap.timeline()
                .to(modalRef.current, {right: "0%", duration: 1, ease: Power3.easeInOut})
        }
        else {
            gsap.timeline()
                .to(modalRef.current, {right: "-50%", duration: 1, ease: Power3.easeInOut})
        ...
        <div ref={modalRef} className={classnames(styles.LinkModal, className)}>
            ...
            {!(data === null) && 
             data.paragraphs.slice(0, 8).map((el: any) => {
                return <div onClick={() => switchToNote(el.id)} className={styles.externallink}>{el.contents}</div>
             })
            }
